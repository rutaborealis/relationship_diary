...
- [x] **T-5** — `backend/src/functions/auth/confirm-reset.ts`: валидация полей + пароль ≥8, `getItem` кода, TTL, timing-safe сравнение хэшей, атомарный `ADD attempts` с `ConditionExpression attempts < :max` (удаление кода при исчерпании), атомарный `SET passwordHash, tokenVersion = if_not_exists(tokenVersion,:1)+:1`, `deleteItem` кода, best-effort письмо «пароль изменён». Один обобщённый текст ошибки `Неверный или истёкший код`.
- [x] **T-6** — `login.ts` кладёт `tv: profile.tokenVersion ?? 1`; `verify-email.ts` — `tv: 1`.
- [x] **T-7** — `backend/config/app.config.ts`: `resetCodeLength` 6, `resetCodeTtlMin` 15, `resetMaxAttempts` 5, `resetRateLimitPerHour` 5, `resetResponseFloorMs` 600.
